...
# 进行可视化确认
现在已经跑起来了CTR-GCN了（看起来是），但是保险起见，最好做一下可视化，看看骨架是否正常而不是错乱。
可视化出来了：根本不像人。👉不过我们可以确定，应该是preprocess_pd出了问题，因为目前为止只用到了这些部分

其实只是连接错了，现在输出了结果，应该没啥问题了。


![](results/visualize/sample_check.gif)

![](results/visualize/sample_check_01.gif)
