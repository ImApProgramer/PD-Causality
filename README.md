...
❓一个重要思考方向：如何让模型自己“感知”如何去做建模？尽可能减少手工定义和引导。

具体来说，混淆变量的感知，特征的感知，约束的权重自动调整，等等各方面。
...
10.08~10.09
论文：
因果建模用的是反事实+granger因果，
granger因果这个之前介绍过，是需要在时间序列上，比较移除了x和没有移除x的情况下，对于未来的某个时刻y的预测结果的影响，如果移除了x之后y的预测准确率下降，我们**说x是y的granger原因**。
这里是进行了一个迁移，在特征空间上进行操作，也就是**比较的是使用完整特征x预测Y和使用干预（利用掩码）后的特征x̄预测Y的损失**。
...
从模型本身来说，混淆分支的五个头分别预测五个标签是**过度约束**，尤其是身高、体重、性别这几个，

相关性是一个重要的参考，因为相关性高是有可能成为混淆变量的**必要条件**。
如果相关性较低，模型根本也没办法将它们作为捷径进行参考（因为哪怕模型能提取相关信息，对预测也无用甚至反作用），此时要求混淆分支强行预测它们，是没有益处的。因为让模型能够进行预测这些混淆变量是一种很强的显式分离手段，但是既然相关性低，去预测它们就是不合理的。

这些任务甚至可能相互干扰。

另外也要考虑到这些混淆因素，是否真的**明显到能在步态中表现出来**？是否**接近帕金森步态的症状**？它们甚至**可能并不是主要的混淆因素**，让模型去预测它们会让模型的注意力从真正重要的混淆因素中转移开来。



**身份**依然是要考虑的因素，**用药状态**必须两个都一起使用：因为如果单纯使用ON/OFF的一种状态，则每个SUB都只会有一个评分，那么模型只要简单学会判别这个人的身份即可“作弊”，但对于LOSOCV来说，这个人它从没见过，因此依然模型学会走这种捷径，泛化性就会非常差。

依然要考虑让模型不能学会判别这个人的身份。



-----

接下来的问题是：对于相关性较高的两个指标（BMI-0.49，年龄-0.32），如何确定模型**真的利用了它们作为捷径**呢？从领域知识来说，它们并不会影响医生的判断，这说明模型**在评分时也不应该考虑它们，而且应该能够区分出来步态特征中它们的影响造成的部分与帕金森造成的部分**。
...
另外，上面的那篇思路的参考，可以通过“与自己评分一样的人”作为正样本，“与自己评分不一样的人”作为负样本，而骨架通过某种相似度计算来作为代理变量。

核心挑战在于：如何定义一个合适的代理变量，能够更好地综合起这些混淆变量，它需要满足以下特点：



**对于病理特征的改变不敏感**，因为它主要反映非病理因素

> 一个**评分不同**的人 N 的混淆特征，比**所有评分相同**的人 P 的混淆特征**离 A 更近**。
>
> 这就可以认为是这两者共享了比较接近的混淆变量。
...
2. 特征距离的度量使用什么？欧几里得距离？余弦距离？

   **余弦相似度**(归一化之后计算点积）

3. 如何选择样本？受硬件限制，batch是无法太大的，据此的解决方案可以采用memory bank。原文是怎么解决的？

   原文用的是PK-RP，默认在mini-batch内寻找，寻找与锚点同ID的样本作为正样本集合，遍历batch内其他个体的样本，

   用**来自数据集剩余部分的随机序列进行填充batch内空间**。
...
|==========BEST REPORTS============
           ...
           0       0.52      0.60      0.56      1026
           1       0.41      0.34      0.37       828
           2       0.31      0.29      0.30       486

    accuracy                           0.45      2340
   macro avg       0.41      0.41      0.41      2340
weighted avg       0.44      0.45      0.44      2340
...
==========BEST REPORTS============
           ...
           0       0.57      0.72      0.64      1026
           1       0.45      0.36      0.40       828
           2       0.24      0.19      0.21       486

    accuracy                           0.48      2340
   macro avg       0.42      0.42      0.41      2340
weighted avg       0.46      0.48      0.46      2340
...
==========BEST REPORTS============
           ...
           0       0.58      0.72      0.64      1026
           1       0.58      0.44      0.50       828
           2       0.32      0.29      0.31       486

    accuracy                           0.53      2340
   macro avg       0.49      0.48      0.48      2340
...
==========BEST REPORTS============
    ...
    accuracy                           0.59     2340
   macro avg       
weighted avg       0.59      0.59      0.59      2340
...
目前的memory bank是只有在训练集上面寻找的，

而测试集中已经使用掉一个个体，

验证集中则是0、1、2三个评分的个体都各自选择两个，

那么1、2类别，留给我们进行挖掘的样本会不会不足？能不能使用验证集的一些样本？



----

先把消融和baseline跑明白，因果要带来改进。多跑几次。

每次只加一个模块。



混淆特征编码要用来干嘛？搞清楚，否则只是一个信息冗余，不会有任何作用。



GRL要用DIFFER那一篇的代码来参考。

对比学习的框架要怎么用，参考什么。



讲解的时候要有格式化：他里面是如何做的，我为什么要搬过来用，为了解决什么问题，我能解决什么问题，我用的方法和它有什么不同。
...
1. 先用完全原封不动的24-FG源代码跑一遍预测，`train_mode`为`end2end`，`seed=1`。

2. 然后，测试加上z_g分支，注意，依然采用原始的分类头，改装一下接到z_g后面，适配其输入

   z_g的写法要好好参考一些文章，例如GaitSCM的。然后**决定是否要启用**

3. z_g后接序回归头，看看效果，**决定是否要启用**

\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=\=

**消融E2-解耦模块**

如果以上z_g及序回归确实带来提升（但是要怎么提升呢？），作为baseline：

1. 加上z_c分支->通常来说不会有提升，因为仅仅是冗余的信息
2. 再加上GRL
3. 再加上Recon
4. 再加上Lct
...
**如何保证“分离”？也就是，你的z_c如何真正对于主任务分类有作用，而不是纯粹的信息冗余。**

有几个选择：

1. 预测混淆变量

   1. 看看如何实现更合适，参考其实现方法，不要手搓
