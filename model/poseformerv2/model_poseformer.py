...
class PoseTransformerV2(nn.Module):
    ...
    def forward_features(self, x, Spatial_feature):
        b, f, p, _ = x.shape
        num_coeff_kept = self.num_coeff_kept                                #正余弦变换，前面几个系数是低频，后面是高频

        x = dct.dct(x.permute(0, 2, 3, 1))[:, :, :, :num_coeff_kept]        #DCT要求在时间维度上做，时间必须是最后一维，并且这里保留前9个系数
        x = x.permute(0, 3, 1, 2).contiguous().view(b, num_coeff_kept, -1)  #然后又变回原来的结构
    ...
    def forward(self, x, return_rep=True):
        b, f, p, _ = x.shape
        x_ = x.clone()
        Spatial_feature = self.Spatial_forward_features(x)      #空间特征
        x = self.forward_features(x_, Spatial_feature)  #融合后的时空特征
